  ...
  useEffect(() => {
    const fetchGitHubStats = async () => {
      try {
        ...
        // 获取缓存的GitHub统计数据
        let response = await fetch('https://raw.githubusercontent.com/slatejack/slatejack-website/refs/heads/master/public/data/github-stats.json');
        if (!response.ok) {
          response = await fetch('/data/github-stats.json');
          if (!response.ok) {
